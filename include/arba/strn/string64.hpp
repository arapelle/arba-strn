...
public:
     ...
     * @brief string64
     */
    constexpr explicit string64() : integer_(0) {}

    template <class Enum>
        requires is_enum64_v<Enum>
    constexpr explicit string64(Enum value) : integer_(static_cast<string64::uint>(value))
    {
    }
     ...
     */
    constexpr explicit string64(const std::string_view& str) : string64()
    {
     ...
     */
    constexpr explicit string64(const std::string& str) : string64(std::string_view(str)) {}
    ...
    constexpr string64(const T& cstr)
        : cstr_{ cstr[0],
                 Tr::template nth<1>(cstr),
                 Tr::template nth<2>(cstr),
                 Tr::template nth<3>(cstr),
                 Tr::template nth<4>(cstr),
                 Tr::template nth<5>(cstr),
                 Tr::template nth<6>(cstr),
    ...
    explicit string64(const T& cstr) : string64(std::string_view(cstr))
    {
    }
    ...
    template <class Enum>
        requires is_enum64_v<Enum>
    inline constexpr Enum to_enum()
    {
        return static_cast<Enum>(integer_);
    }
...
private:
    ...
    constexpr explicit string64(uint value) : integer_(value) {}
...
/**
 * @brief The std::hash<strn::string64> struct specialization
 */
template <>
struct std::hash<arba::strn::string64>
{
    inline std::size_t operator()(const arba::strn::string64& value) const noexcept { return value.hash(); }
};

/**
 * @brief The std::formatter<::arba::strn::string64> struct specialization
 */
template <class CharT>
struct std::formatter<::arba::strn::string64, CharT> : public std::formatter<std::string_view, CharT>
{
    template <class FormatContext>
    auto format(const ::arba::strn::string64& str, FormatContext& ctx) const
    {
